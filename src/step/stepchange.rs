...
impl StepChange {
    ...
    pub fn apply_moduleblockchange(&self, hosthandler: &mut HostHandler) -> StepResult {
        ...
        for api_call_result in step_result.apicallresults.iter() {
            if let Some(old_output) = &api_call_result.output {
                new_api_call_results_list.push(
                    ApiCallResult::from(
                        api_call_result.rc,
                        Some(old_output
                            .chars()
